...
class MeasureAppliedForce(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        self.logger.debug(f"{self.name} [MeasureAppliedForce::setup()]")

    def initialise(self):
        self.logger.debug(f"{self.name} [MeasureAppliedForce::initialise()]")
        self.start_time = self.data.time
        

    def update(self):
        h_e = action_utils.get_fortor_ee(self.model, self.data, self.robot)
        print(h_e)
        return py_trees.common.Status.RUNNING

    def terminate(self, new_status):
        self.logger.debug(f"{self.name} [MeasureAppliedForce::terminate()] -> {new_status}")
