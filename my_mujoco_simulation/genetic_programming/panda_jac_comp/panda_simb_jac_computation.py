...
# -------------------------------------------------------------
# NEW FUNCTION: derivative of Jacobian wrt q[k]
# -------------------------------------------------------------
def get_dJdq(joint_angles, q_index, eps=1e-6):
    """
    Returns the derivative of the Jacobian wrt q[q_index]:
    dJ/dq_i  (6x7 matrix)
    """

    # baseline
    J0 = get_jacobian(joint_angles)

    # perturb
    q_perturbed = np.array(joint_angles, dtype=float)
    q_perturbed[q_index] += eps

    J1 = get_jacobian(q_perturbed)

    # finite difference: (J(q+eps) - J(q)) / eps
    dJdq_i = (J1 - J0) / eps
    return dJdq_i